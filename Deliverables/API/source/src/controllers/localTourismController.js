...
const getNearbyPlaces = async (req, res, next) => {
  try {
    ...
    if (req.user?.id) {
      ...
      const favorites = await Favorite.find({ userId: req.user.id, placeId: { $in: placeIds } }).lean();
      const favoriteMap = new Map(favorites.map((fav) => [String(fav.placeId), fav._id]));
...
const getPlaceById = async (req, res, next) => {
  try {
    ...
    ActivityLog.create({
      ...
      source: req.headers['x-client-platform'] || 'mobile',
      metadata: { placeId: place._id, placeName: place.name },
    }).catch(() => {});

    if (req.user?.id) {
      const favorite = await Favorite.findOne({ userId: req.user.id, placeId: place._id }).lean();
