...
const updateItinerary = async (req, res, next) => {
  try {
    ...
    // Notifica en tiempo real a quien tenga este itinerario abierto en
    // otra plataforma (Web / App Móvil / Smartwatch comparten cuenta vía
    // JWT). Si Socket.IO no está inicializado (ej. tests con supertest,
    // que importan src/app.js sin pasar por index.js) simplemente se
    // omite, sin afectar la respuesta HTTP.
    try {
      getIO().to(`itinerary:${itinerary.id}`).emit('itinerary:updated', { itinerary });
    } catch {
      // Socket.IO no inicializado en este contexto (ej. entorno de test).
    }
