 ...
 *   5. Arranque del servidor HTTP Express + Socket.IO.
 *
 ...
 *
 * Nota sobre sockets: se usa http.createServer(app) en vez de app.listen()
 * directo para poder adjuntar Socket.IO al MISMO servidor HTTP (mismo
 * puerto, sin proceso aparte). Ver src/sockets/index.js.
 * -----------------------------------------------------------------------
 */
...
const startServer = async () => {
  try {
    ...
    // 3. Servidor HTTP + Socket.IO sobre el mismo puerto
    const server = http.createServer(app);

    const io = new Server(server, {
      cors: { origin: '*' }, // TODO: restringir origin en producción
    });

    initSockets(io);

    server.listen(PORT, () => {
      logger.info(`🚀 TraveXperience API + sockets corriendo en el puerto ${PORT} (${process.env.NODE_ENV})`);
    });
