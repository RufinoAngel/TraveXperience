...
npm run migrate:entity-fields  # (una sola vez) migra favoritos/reseñas viejas al esquema entityType/entityId
...
`GET http://192.168.90.11:4000/api/v1/health`
...
`GET /?entityType=&entityId=`, `POST /` `{entityType, entityId, entityName, rating, comment, photos, tags}`,
`PUT /:id`, `DELETE /:id`. `entityType` es `"place"` o `"hotel"`. Cada cambio recalcula
`ratingAvg`/`ratingCount` en la entidad correspondiente (`Place` u `Hotel`), para lectura rápida
desde la App Móvil y el Smartwatch.
...
| Método | Ruta | Descripción |
|---|---|---|
| GET | `/?entityType=` | Entidades favoritas del usuario autenticado (place u hotel), con el detalle ya incluido (populate dinámico) |
| POST | `/` `{entityType, entityId}` | Agrega una entidad a favoritos (idempotente: si ya existe, no duplica) |
| DELETE | `/:entityId?entityType=` | Quita una entidad de favoritos |
...
- Conectar un proveedor real de correo para `forgot-password` (SendGrid/SES).
- Entrenar y colocar los modelos `.json`/`.bin` en `src/ai-models/` para activar
  `aiService.predictBudget` / `predictTravelerCluster` en producción.
- Añadir tests de integración (Jest + supertest) por módulo.
- Documentar la API con OpenAPI/Swagger para el equipo de Kotlin y React Native.
