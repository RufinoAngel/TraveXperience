...
/**
 * GET /local/places
 * Listado general paginado del catálogo de lugares (sin exigir lat/lng),
 * usado por el panel de Admin para poblar la tabla completa. Admite
 * filtro opcional por categoría y por texto libre en el nombre.
 */
const listPlaces = async (req, res, next) => {
  try {
    ...
    const filter = {};
    // El panel de Admin necesita ver también los lugares inactivos; el
    // resto de consumidores (público) puede pedir ?isActive=true si quiere.
    ...
    return ApiResponse.success(res, 200, 'Lugares obtenidos.', { total, page, pageSize, places });
  } catch (error) {
    return next(error);
  }
};
 ...
 * Solo administradores. Borrado físico del documento, mismo patrón que
...
const deletePlace = async (req, res, next) => {
  try {
    ...
    return ApiResponse.success(res, 200, 'Lugar eliminado.');
  } catch (error) {
    return next(error);
  }
};
...
/**
 * POST /local/places/:id/images
 * Solo administradores. Multipart/form-data, campo "images" (hasta 10).
 * Sube el/los archivo(s) y hace push de las URLs resultantes al arreglo
 * `images` del lugar. Responde con el mismo formato que updatePlace.
 */
const addPlaceImages = async (req, res, next) => {
  try {
    const place = await Place.findById(req.params.id);
    if (!place || !place.isActive) {
      throw new AppError('Lugar no encontrado.', 404);
    ...
    if (!req.files || req.files.length === 0) {
      throw new AppError('Se requiere al menos un archivo de imagen en el campo "images".', 400);
    }

    const uploadedUrls = req.files.map((file) => toPublicUrl(file.path));
    place.images = [...(place.images || []), ...uploadedUrls];
    await place.save();

    return ApiResponse.success(res, 200, 'Imágenes agregadas al lugar.', { place });
  } catch (error) {
    return next(error);
  }
};
...
/**
 * DELETE /local/places/:id/images
 * Body: { imageUrl }. Solo administradores. Quita una imagen puntual del
 * arreglo `images` del lugar (no borra el archivo físico del disco).
 */
const removePlaceImage = async (req, res, next) => {
  try {
    const { imageUrl } = req.body;
    if (!imageUrl) {
      throw new AppError('Se requiere "imageUrl" en el cuerpo de la petición.', 400);
    }
    ...
    place.images = (place.images || []).filter((url) => url !== imageUrl);
    await place.save();

    return ApiResponse.success(res, 200, 'Imagen eliminada del lugar.', { place });
  } catch (error) {
    return next(error);
  }
};
